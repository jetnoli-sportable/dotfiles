...
return {
	...
	config = function()
		...
		-- Run a tmux command, but only when actually inside a tmux session.
		local function tmux(cmd)
			if not vim.env.TMUX or vim.fn.executable("tmux") ~= 1 then
				vim.notify("oil.nvim: not in a tmux session", vim.log.levels.WARN)
				return
			end
			os.execute(cmd)
		end
		...
		opts.keymaps = {
			...
			["<C-o>"] = function()
				...
				if cwd then
					tmux(string.format("tmux split-window -h -c %s", vim.fn.shellescape(cwd)))
				else
