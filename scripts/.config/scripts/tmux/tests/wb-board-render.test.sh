...
# Column 1 starts at 40 + 272 = 312; the widest possible card in column 0
# ends at 40 + 208 = 248, so the line belongs in that gap (at 280).
assert "U4 DAG9b: frontier sits in the gap between columns, not over cards" 'class="dag-frontier" x1="280"' "$dag9b_html"
...
assert "U4 DAG9b: a long short-id is clipped with an ellipsis" 'class="dag-id"[^>]*>a-very-lo[^<]*&#8230;</text>' "$dag9d_html"
...
dag9e_html=""
wb_board_v2_dag_html fam9e DAG9E_NODES DAG9E_LAYER DAG9E_ORDER DAG9E_CRIT DAG9E_STARTABLE \
