...
`/handoff-pane` (a second agent in a split pane of the current tmux window,
sharing this session's worktree) was removed in the 2026-09-14 piece audit —
zero measured usage since it shipped. `/handoff` covers the routing use case:
an already-live session (switch + clipboard) or a freshly spawned one (`wb new
--agent`). If you need a second agent looking at this exact worktree's
uncommitted changes again in the future, that's the gap `/handoff-pane` used
to fill; for now, commit or stash first and route via `/handoff` instead.
