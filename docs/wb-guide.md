...
## /wb-jira-create — file a task or family as Jira tickets

The reverse of `wb breakdown`'s ticket→task path: take a task (or a whole
breakdown family) you scoped locally and **emit** it as new Jira tickets the
team can see — in a chosen project (**SFB** by default, or **SW**, or
another) — each created ticket's URL written back into the task's `jira:`
field.
...
checkbox buffer for you to edit and approve. The buffer carries a run-level
**`Project:`** field (default `SFB`, editable to `SW` or another key you can
reach — the issue types on offer come from that project's own metadata), an
editable per-ticket issue **type** (for SFB: Feature / Defect / Bug /
Improvement / New Feature, default `Feature`), and one run-level
**`Parent ticket:`** field: leave it blank for flat tickets, name an existing
key like `SFB-1234` to link every child to it, or point at a batch row
(default: the family's coordinator) to create that parent first and link the
children to it with a generic "Relates" issue-link. Nothing is created until
you approve; the tickets are created **agent-side over the Atlassian MCP**
(create-only — this never transitions, edits, or updates an existing ticket).
