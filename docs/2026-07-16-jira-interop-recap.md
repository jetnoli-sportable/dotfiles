---
title: Recap — Jira ticket interop (emit tasks to SFB tickets)
status: current
tile: A human-approved proposal buffer plus a locked write-back verb — turning a wb task or /wb-breakdown family into new SFB Jira tickets, with each ticket's URL stamped back into the task.
group: recaps
kind: page
updated: 2026-07-16
---
...
create the SFB tickets from it without leaving the workbench.
...
(creating real SFB tickets) is a manual run, since it depends on a live
